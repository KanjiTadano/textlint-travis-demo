...
var _japaneseNumeralsToNumber = require("japanese-numerals-to-number");

var _japaneseNumeralsToNumber2 = _interopRequireDefault(_japaneseNumeralsToNumber);
...
function reporter(context) {
    ...
    return _defineProperty({}, Syntax.Str, function (node) {
        ...
        // 漢数字 -> 算用数字
        var toNumber = function toNumber(text, pattern, match) {
            var matchedString = match[0];
            var index = match.index;
            var expected = matchedString.replace(pattern, function (all, match) {
                return all.replace(match, (0, _japaneseNumeralsToNumber2.default)(match));
            });
        ...
        var toKanNumber = function toKanNumber(text, pattern, match) {
            var matchedString = match[0];
            var expected = matchedString.replace(pattern, function (all, match) {
        ...
        var ignoreWhenMatched = function ignoreWhenMatched(ignorePattern, matchFn) {
            return function (text, pattern, match) {
        ...
        matchToReplace(text, /([一二三四五六七八九十壱弐参拾百〇]+)[兆億万]/g, ignoreWhenMatched(/数([一二三四五六七八九十壱弐参拾百〇]+)[兆億万]/g, toNumber));
